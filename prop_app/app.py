...
with tab_settings:
    ...
    st.markdown("### Sheet Cavitation (Brockett -Cp_min)")
    s1, s2 = st.columns(2)
    c.sheet_cpmin_thickness_k = s1.number_input("Thickness Factor K", value=c.sheet_cpmin_thickness_k)
    c.sheet_cpmin_loading_k = s2.number_input("Loading Factor K", value=c.sheet_cpmin_loading_k)
    c.sheet_cpmin_ideal_cl = s1.number_input("Ideal CL (zero suction peak)", value=c.sheet_cpmin_ideal_cl)
    c.sheet_severity_scale = s2.number_input("Severity Scale", value=c.sheet_severity_scale)

    st.markdown("### Tip Vortex (McCormick)")
    cc1, cc2 = st.columns(2)
    c.tip_mccormick_K = cc1.number_input("McCormick K", value=c.tip_mccormick_K)
    c.tip_mccormick_m = cc2.number_input("Circulation Exponent m", value=c.tip_mccormick_m)
    c.tip_mccormick_n = cc1.number_input("Reynolds Exponent n", value=c.tip_mccormick_n)
    c.tip_core_radius_coeff = cc2.number_input("Core Radius r_c/c", value=c.tip_core_radius_coeff)
    c.tip_start_rR = cc1.number_input("Tip Region Start r/R", value=c.tip_start_rR)
    c.tip_end_rR = cc2.number_input("Tip Region End r/R", value=c.tip_end_rR)
